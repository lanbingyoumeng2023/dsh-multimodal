...
function getMultimodalKey() {
	...
	return new Promise((resolve, reject) => {
		// 显式带上 Windows PowerShell 5.1 系统模块路径：当宿主进程从 PowerShell 7
		// 启动时，$PSModulePath 不含 5.1 模块目录，ConvertTo-SecureString 会加载失败。
		const env = {
			...process.env,
			PSModulePath: "C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\Modules" + (process.env.PSModulePath ? ";" + process.env.PSModulePath : "")
		};
		// 优先用 pwsh（PowerShell 7）：Microsoft.PowerShell.Security 模块仅存在于 pwsh，
		// powershell.exe（5.1）在某些环境下（如从 pwsh 启动的进程继承的 PSModulePath）找不到该模块。
